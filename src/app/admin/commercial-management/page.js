...
export default function CommercialManagementPage() {
  ...
  // Load commercial campaigns & team from localStorage / API
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedCampaigns = localStorage.getItem('weddingpur_commercial_campaigns');
      if (savedCampaigns) {
        try {
        ...
        // Fallback default sample campaigns if none in storage
        setCampaigns([
          { id: "C-101", title: "Brand Ad Shoot", totalBudget: "₹3,50,000", status: "SCHEDULED", month: "SEP", year: 2026 },
          { id: "C-102", title: "TVC Commercial", totalBudget: "₹5,00,000", status: "DELIVERED", month: "AUG", year: 2026 }
        ]);
      }
    }
  ...
  const totalTargetNum = filteredCampaigns.reduce((acc, c) => acc + parseBudget(c.totalBudget), 0);
  ...
  const busyCrew = team.filter(t => t.isBusy || t.status === 'BUSY').length || 2;
  const freeCrew = Math.max(0, team.length - busyCrew);
  const deliveredCampaigns = campaigns.filter(c => c.status === 'DELIVERED').length;
  const pendingCampaigns = campaigns.length - deliveredCampaigns;
    ...
    <div className="min-h-screen bg-[#0B0D0E] text-[#F5F5F5] font-sans antialiased p-6 lg:p-10 space-y-8 selection:bg-[#D4AF37] selection:text-black">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div className="bg-[#121518] border border-[#2B2519] rounded-3xl p-5 shadow-xl space-y-1.5 relative overflow-hidden">
          ...
          <p className="text-[11px] font-sans text-[#A89D84] font-medium">
            {filteredCampaigns.length} Active Campaigns in this period
          </p>
        </div>

        {/* CARD 2: REAL ACTIVE CAMPAIGNS */}
        <div className="bg-[#121518] border border-[#2B2519] rounded-3xl p-5 shadow-xl space-y-1.5 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 bg-emerald-500/5 rounded-full blur-xl pointer-events-none"></div>
          <span className="text-[10px] font-sans font-semibold text-[#8A7D5C] uppercase tracking-wider block">
            Active Commercial Contracts ({timeFilter === '1M' ? 'September 2026' : timeFilter === '6M' ? 'Last 6 Months' : 'Full Year 2026'})
          </span>
          <div className="text-3xl font-sans font-bold tracking-tight text-emerald-400 font-mono">
            {filteredCampaigns.length} Campaigns
          </div>
          <p className="text-[11px] font-sans text-[#A89D84] font-medium">
            Fully synced with Brand Dispatch Ledger
          </p>
        </div>
      ...
      <div className="pt-4 space-y-6">
        <div>
          ...
          <p className="text-xs font-sans text-[#8A7D5C] mt-1 font-normal">Manage personnel, monitor availability, and track live brand deployments.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          
          <Link className="group relative bg-[#121518] border border-[#2B2519] hover:border-[#D4AF37] rounded-3xl p-7 shadow-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_30px_rgba(212,175,55,0.15)] flex flex-col justify-between space-y-6 block cursor-pointer" href="/admin/commercial-team">
          ...
          <Link className="group relative bg-[#121518] border border-emerald-500/30 hover:border-emerald-400 rounded-3xl p-7 shadow-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_30px_rgba(52,211,153,0.15)] flex flex-col justify-between space-y-6 block cursor-pointer" href="/admin/commercial-team/available">
            <div className="space-y-4">
              ...
              <div>
                ...
                <span className="text-[10px] font-sans font-semibold text-emerald-400/80 uppercase tracking-wider block mb-2 mt-1">STATUS: {freeCrew} Available</span>
                <p className="text-xs font-sans text-[#A89D84] font-normal leading-relaxed">Personnel with zero conflicting bookings. Directly assign these available directors and DPs to new brand campaigns.</p>
              </div>
            </div>
            <div className="pt-4 border-t border-[#1C1F24]">
          ...
          <Link className="group relative bg-[#121518] border border-rose-500/30 hover:border-rose-400 rounded-3xl p-7 shadow-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_30px_rgba(251,113,133,0.15)] flex flex-col justify-between space-y-6 block cursor-pointer" href="/admin/commercial-team/busy">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
              ...
              <div>
                ...
                <p className="text-xs font-sans text-[#A89D84] font-normal leading-relaxed">Track personnel currently out on field missions. Monitor their exact commercial assignments, locations, and return dates.</p>
              </div>
            </div>
            <div className="pt-4 border-t border-[#1C1F24]">
      ...
      {/* SECTION 2: BRAND CLIENT OPERATIONS */}
      <div className="pt-4 border-t border-[#1F242D] space-y-6">
        <div>
          <h2 className="text-xl sm:text-2xl font-extrabold tracking-tight text-white font-sans">Brand Client Operations Hub</h2>
          <p className="text-xs font-sans text-[#8A7D5C] mt-1 font-normal">Manage corporate ad campaigns, deliverables, active contracts, and brand financials.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          
          <Link className="group relative bg-[#121518] border border-[#2B2519] hover:border-[#D4AF37] rounded-3xl p-7 shadow-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_30px_rgba(212,175,55,0.15)] flex flex-col justify-between space-y-6 block cursor-pointer" href="/admin/commercial/dispatch">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="w-12 h-12 rounded-2xl bg-[#1A1E24] border border-[#2B2519] flex items-center justify-center text-xl group-hover:scale-110 group-hover:border-[#D4AF37] transition-all">
                  🎬
              ...
              <div>
                <h3 className="text-base font-sans font-semibold tracking-tight text-white group-hover:text-[#D4AF37] transition-colors">
                  All Commercial Campaigns
                </h3>
                <span className="text-[10px] font-sans font-semibold text-[#8A7D5C] uppercase tracking-wider block mb-2 mt-1">STATUS: Master Dispatch</span>
          ...
          <Link className="group relative bg-[#121518] border border-emerald-500/30 hover:border-emerald-400 rounded-3xl p-7 shadow-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_30px_rgba(52,211,153,0.15)] flex flex-col justify-between space-y-6 block cursor-pointer" href="/admin/commercial/dispatch">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="w-12 h-12 rounded-2xl bg-emerald-950/40 border border-emerald-500/40 flex items-center justify-center text-xl group-hover:scale-110 transition-all text-emerald-400">
                  ✓
                ...
                <span className="text-[10px] font-mono font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 px-2.5 py-1 rounded-lg">
                  {deliveredCampaigns} Completed
                </span>
              </div>

              <div>
                <h3 className="text-base font-sans font-semibold tracking-tight text-white group-hover:text-emerald-300 transition-colors">
          ...
          <Link className="group relative bg-[#121518] border border-amber-500/30 hover:border-amber-400 rounded-3xl p-7 shadow-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_30px_rgba(251,191,36,0.15)] flex flex-col justify-between space-y-6 block cursor-pointer" href="/admin/commercial/dispatch">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-mono font-bold text-amber-400 bg-amber-500/10 border border-amber-500/30 px-2.5 py-1 rounded-lg animate-pulse">
                  {pendingCampaigns} Active Pipeline
                </span>
              </div>

              <div>
                <h3 className="text-base font-sans font-semibold tracking-tight text-white group-hover:text-amber-300 transition-colors">
