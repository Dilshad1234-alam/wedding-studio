...
function CrewDirectoryContent() {
  ...
  // Dynamic Client Shoots Roster Database
              ...
              client.schedule.forEach(day => {
                ...
                roles.forEach(role => {
                  ...
                  if (crewName && crewName !== '—' && crewName !== '-') {
                    // split comma separated names if multiple assigned to same role
  ...
  const getAssignmentsForCrew = (memberName) => {
    ...
    return allClientAssignments.filter((a) => {
      // Use includes for partial matching (e.g. if memberName is "Sanoj Kr LLP" and assigned name is "Sanoj")
