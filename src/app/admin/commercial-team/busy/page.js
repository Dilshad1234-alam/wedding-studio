...
function BusyCrewContent() {
  ...
  // 2. Master Brand/Commercial Shoots Roster Database
  ...
  const handleMonthChange = (monthValue) => {
    ...
    if (monthValue === 'ALL') {
      router.replace('/admin/commercial-team/busy', { scroll: false });
    } else {
      router.replace(`/admin/commercial-team/busy?month=${monthValue}`, { scroll: false });
    }
  };
    ...
    <div className="min-h-screen bg-[#0B0D0E] text-[#F5F5F5] font-sans antialiased p-6 lg:p-10 space-y-6 selection:bg-rose-500 selection:text-white">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[#1F242D]">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Link className="px-4 py-2.5 rounded-xl border border-emerald-500/30 bg-[#121518] hover:border-emerald-400 text-emerald-400 text-xs font-sans font-semibold uppercase tracking-wider transition-all shadow-md" href="/admin/commercial-team/available">
                  ...
                  <td colSpan={8} className="py-12 text-center text-xs font-mono text-[#8A7D5C]">
                    <div className="flex flex-col items-center justify-center space-y-2">
                      ...
                      <p className="text-[#8A7D5C] text-xs font-sans">
                        All crew members are free or no shoots are logged for this period. Check the <Link href="/admin/commercial-team/available" className="text-emerald-400 underline">Available Bench</Link>.
                      </p>
                    </div>
                  </td>
                </tr>
